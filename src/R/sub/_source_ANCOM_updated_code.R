...
ancom.W = function(otu_data,var_data,
  ...
  for(ii in 1:(n_otu-1)){
    for(jj in (ii+1):n_otu){
      ...
      data.pair <- # Numerator and Denominator
        data_comp[
          ,
          which(
            colnames( data_comp) %in%
              otu_ids[c( ii, jj)]
            )
          ]
      ...
      if(logratio.mat[ii,jj] < sig) print(list(colnames(data.pair), logratio.mat[ii,jj])) 
      }
    } 
  ...
  print(surr.pval) # added for debug 19/06/18
  #########################################
  ### Conservative
  if(multcorr==1){
...
ANCOM.main = function(OTUdat,Vardat,
  ...
  W.detected   <- ancom.W(
    OTUdat.thinned,Vardat,adjusted,repeated,
    main.var,adj.formula,repeat.var,longitudinal,
    random.formula,multcorr,sig
    )
  ...
  W_frame$detected_0.5=rep(FALSE,dim(W_frame)[1])
  W_frame$detected_0.4=rep(FALSE,dim(W_frame)[1])
  W_frame$detected_0.3=rep(FALSE,dim(W_frame)[1])
  W_frame$detected_0.2=rep(FALSE,dim(W_frame)[1])
  W_frame$detected_0.1=rep(FALSE,dim(W_frame)[1])
  
  W_frame$detected_0.9[
    which(
      W_frame$W_stat > 0.9 * (
        dim(OTUdat.thinned[,-1])[2]-1)
      )
    ] = TRUE
  ...
  W_frame$detected_0.5[which(W_frame$W_stat>0.5*(dim(OTUdat.thinned[,-1])[2]-1))]=TRUE
  W_frame$detected_0.4[which(W_frame$W_stat>0.4*(dim(OTUdat.thinned[,-1])[2]-1))]=TRUE
  W_frame$detected_0.3[which(W_frame$W_stat>0.3*(dim(OTUdat.thinned[,-1])[2]-1))]=TRUE
  W_frame$detected_0.2[which(W_frame$W_stat>0.2*(dim(OTUdat.thinned[,-1])[2]-1))]=TRUE
  W_frame$detected_0.1[which(W_frame$W_stat>0.1*(dim(OTUdat.thinned[,-1])[2]-1))]=TRUE
  
  
  final_results=list(W_frame,zero.plot,W_frame.ori)
  names(final_results)=c("W.taxa","PLot.zeroes","W_frame.ori")
