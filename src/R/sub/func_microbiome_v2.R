...
adonis_simple <- function(
  model,
  binary=FALSE,
  method="bray"
){
  permanova_res <- vegan::adonis(
    formula = as.formula(model),
    method = method,
    permutations = how(nperm = 20000),
    binary=binary
  )
  return(
    permanova_res
...
func.data_for_heatmap_v4 <- function(
  ...
  method.dist.row = 'euclidean',
  method.dist.col = 'euclidean',
  method.hclust.row = 'centroid',
  method.hclust.col = 'centroid',
  fn.meth.filt.taxa = "_"
  ...
  quartz(
    type="pdf", 
    file=sprintf(
      fmt = "%s/Heatmap_log_rowProp_%s_%s_%s_color_%s_bias_%s%s.pdf",
      dir.output,
      aggTaxonLvl,
      rareTrimming,
      fn.out.fix,
  ...
  axis(
    ...
    labels=round(breaks,  round.leg.labels)
  )
  dev.off()
}
...
func.data_for_heatmap_v5 <- function(
  ...
  n.breaks    = 20,
  col.bias    = 2,
  colBar.annotations = factor(pData(obj.aggTaxa)$Disease),
  colBar.vec.of.col  =  c('red', 'green', 'gray'),
  breaks      = NULL,
  round.leg.labels = 2,
  row.adjust  = 0,
  col.adjust  = 0,
  ...
  col.cex.adjust = 1,
  obj.aggTaxa,
  expr.heatmap = quote(x/sum(x)),
  expr.hclust.row = quote(x),
  expr.hclust.col = quote(x),
  na.val=0,
  ...
  col_1 = colorRampPalette(
    # c("green4", "red"),                                                           # ver.2_red_green
    c(paste0("gray", seq(breaks.from,breaks.to,length.out = 2)[order(seq(breaks.from,breaks.to,length.out = 2),decreasing=TRUE)])),
    space = "rgb",
  ...
  OTUdata_all.transformed  <- 
    OTUdata_all %>%
    data.frame() %>%
    apply(
      2, 
      function(vec){
        eval(expr.heatmap, list(x=vec))
      }
    ) %>%
    data.frame()
    ...
    as.matrix(
        OTUdata_all %>%
          ...
          apply(
            2, 
            function(vec){
              eval(expr.hclust.row, list(x=vec))
            }
          ) %>%
          data.frame()
        ),
        ...
        as.matrix(
          OTUdata_all %>%
            ...
            apply(
              2, 
              function(vec){
                eval(expr.hclust.col, list(x=vec))
              }
            ) %>%
            data.frame()
          )
  ...
  colv.hc$height <- (colv.hc$height)**5
  ...
  if(is.null(breaks)){
    all_num <- unique( 
      unlist(c(OTUdata_all.transformed))
      )
  ...
  mapCharToColor <- 
    function(
      annotations = colBar.annotations,
      vec.of.col  = colBar.vec.of.col
      ){
  ...
  OTUdata_all.transformed[
    OTUdata_all.transformed==na.val
    ] <- NA
  
  quartz(
    type="pdf",
    file=sprintf(
      fmt = "%s/%s_%s_%s_%s_%s_color_%s_bias_%s.pdf",
      dir.output,
      fn.option,
      aggTaxonLvl,
      rareTrimming,
  ...
  nr <- nrow(OTUdata_all.transformed) + row.adjust
  nc <- ncol(OTUdata_all.transformed) + col.adjust
  
  hmAll <- 
    heatmap.2(
      as.matrix(OTUdata_all.transformed),
      ColSideColors=sampleColors,
      Rowv = rowv,
      Colv = colv,# _AAV,
      col = c(col_1(n.breaks)),
      na.color = "white",
      scale = c("none"),
      trace=c("none"),# level trace
      cexRow = (1.4*row.cex.adjust)/log10(nr*10),
      cexCol = 0.2 + col.cex.adjust/log10(nc),
      density.info=c("none"),
      key = T,
      #      cexRow=1,
      breaks = breaks,
  ...
  plot(phylo.colv, type="fan", tip.color=sampleColors, cex=3)
  ...
  # par(omi = c(0,5,0,0))
  # plot(NULL, xlim=c(0,0.1), ylim=c(0,(n.breaks+1)), axes=FALSE, xlab="", ylab="")
  # rect( 0, 1:n.breaks, 0.1, 2:(n.breaks), col = c(col_1(n.breaks)), border=NA)
  # axis(
  #   side=2, 
  #   at = 1:(n.breaks+1), 
  #   las = 1, # always horizontal,
