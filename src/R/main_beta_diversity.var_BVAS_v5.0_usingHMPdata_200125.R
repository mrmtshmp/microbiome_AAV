...
vect.fn.HMP_res.ANCOM <- c(
  ...
  "featuredata_original.FAMILY.csv",
  "featuredata_original.GENUS.csv"
)
...
for(i in 2:length(df.microbePhysiol_csv)){
  ...
  obj.aggTaxa.ADS <- newMRexperiment(
    counts = filt.MRcounts.obj.aggTaxa.ADS[
      ,
      colSums(filt.MRcounts.obj.aggTaxa.ADS)>0 &
        pData(obj.aggTaxa.ADS)$Disease == 'AAV' # BVAS is scoring for AAV patients.
      ],
    phenoData = AnnotatedDataFrame(pData(obj.aggTaxa.ADS)[
      colSums(filt.MRcounts.obj.aggTaxa.ADS)>0 &
        pData(obj.aggTaxa.ADS)$Disease == 'AAV' # BVAS is scoring for AAV patients.
        ,
      ]),
  ...
  res.PERMANOVA <- adonis_simple(
    "dist.obj.aggTaxa.ADS ~ as.numeric(pData(obj.aggTaxa.ADS)$BVAS)"
    )
  ...
  tmp.list.gg_PCoA_overlay.FALSE <- ExploratoryDataAnalysis::gg_PCoA(
    ...
    as.numeric(pData(obj.aggTaxa.ADS)$BVAS),
    ...
    title=sprintf(
      ...
      p= %s (PERMANOVA) (association between BVAS and dissimilarity)",
      method.dist.col,
      select_microbePhysiol,
      round(
...
pdf(
  file = sprintf(
      "%s/PCoA_%s_.var_BVAS.distance.agg.taxon_%s.%s.pdf",
      dir.Output,
      method.dist.col,
      aggTaxonLvl,
      fn.microbePhysiol_csv
